...
const Home = () => {
  ...
  useEffect(() => {
    // Check if user is genuinely logged in by checking session flag
    // This flag should only be set when user successfully logs in and navigates to dashboard
    ...
    if (isActiveSession && token) {
      ...
      // Clear any old tokens and ensure we show "Get Started"
    ...
    <div className="font-sans">
      ...
      <div className="relative h-screen bg-cover bg-center" style={{ backgroundImage: "url('../src/assets/studio.jpg')" }}>
        ...
        <div className="relative flex flex-col items-center justify-center h-full text-center text-white p-4">
          ...
          {isLoggedIn ? (
            <Link to="/booking" className="bg-blue-600 text-white px-8 py-3 rounded-full text-lg font-semibold hover:bg-blue-700 transition-colors">
              Book Now
            </Link>
          ) : (
      ...
      <section className="bg-blue-600 text-white py-16 text-center">
        <div className="container mx-auto px-4">
          ...
          {isLoggedIn ? (
            <Link to="/booking" className="bg-white text-blue-600 px-8 py-3 rounded-full text-lg font-semibold hover:bg-gray-200 transition-colors">
              Book Now
            </Link>
          ) : (
            <Link to="/login-form" className="bg-white text-blue-600 px-8 py-3 rounded-full text-lg font-semibold hover:bg-gray-200 transition-colors">
              Get Started
            </Link>
          )}
        </div>
